...
export const ModelSettings = ({ isDarkMode = false }: ModelSettingsProps) => {
    ...
    // State for test connection functionality
    ...
    const getButtonProps = (provider: string) => {
        ...
        if (providerType === ProviderTypeEnum.CustomOpenAI) {
            hasInput = Boolean(config?.baseUrl?.trim()) && Boolean(config?.modelNames?.length); // Custom needs Base URL and at least one model
    ...
    const handleSave = async (provider: string) => {
        try {
            ...
            // Check if custom OpenAI provider has model names
            if (
                providers[provider].type === ProviderTypeEnum.CustomOpenAI &&
                (!providers[provider].modelNames || providers[provider].modelNames.length === 0)
            ) {
                alert('Custom OpenAI providers must have at least one model name. Please add a model in the Models section.');
                return;
            }
    ...
    const handleRestEndpointChange = (provider: string, restEndpoint: string) => {
        setModifiedProviders(prev => new Set(prev).add(provider));
        setProviders(prev => ({
            ...prev,
            [provider]: {
    ...
    const handleTestConnection = async (providerId: string) => {
        ...
        setConnectionResults(prev => ({ ...prev, [providerId]: { success: false } }));

        try {
            // Build the test URL manually (including REST endpoint)
            // NOTE: This is different from LangChain ChatOpenAI which automatically appends /chat/completions
            let testUrl = config.baseUrl;

            // Add REST endpoint if provided
            if (config.restEndpoint) {
                const baseEndsWithSlash = testUrl.endsWith('/');
                const endpointStartsWithSlash = config.restEndpoint.startsWith('/');

                if (baseEndsWithSlash && endpointStartsWithSlash) {
                    testUrl = testUrl + config.restEndpoint.substring(1);
                } else if (!baseEndsWithSlash && !endpointStartsWithSlash) {
                    testUrl = testUrl + '/' + config.restEndpoint;
                } else {
                    testUrl = testUrl + config.restEndpoint;
                }
            }
            ...
            if (config.customQuery && Object.keys(config.customQuery).length > 0) {
                const url = new URL(testUrl);
                Object.entries(config.customQuery).forEach(([key, value]) => {
            ...
            // Make a simple test request (we'll use fetch with minimal options)
            ...
            if (response.ok || response.status === 401 || response.status === 403) {
                // Consider 401/403 as "connection successful" since it means the server responded
    ...
    const handleCustomHeadersChange = (provider: string, headers: Record<string, string>) => {
        setModifiedProviders(prev => new Set(prev).add(provider));
        setProviders(prev => ({
            ...prev,
            [provider]: {
        ...
        // Don't update headerTextValues here - let the textarea manage its own display value
    };

    const handleCustomQueryChange = (provider: string, query: Record<string, string>) => {
        setModifiedProviders(prev => new Set(prev).add(provider));
        setProviders(prev => ({
            ...prev,
            [provider]: {
        ...
        // Don't update queryTextValues here - let the textarea manage its own display value
    };
                            ...
                            return (
                                <div
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {/* REST Endpoint input (for OpenAI-compatible providers) */}
                                        {(providerConfig.type === ProviderTypeEnum.CustomOpenAI ||
                                            providerConfig.type === ProviderTypeEnum.Ollama ||
                                            providerConfig.type === ProviderTypeEnum.AzureOpenAI ||
                                            providerConfig.type === ProviderTypeEnum.OpenRouter ||
                                            providerConfig.type === ProviderTypeEnum.Llama) && (
                                                <div className="flex flex-col">
                                                    <div className="flex items-center">
                                                        <label
                                                            htmlFor={`${providerId}-rest-endpoint`}
                                                            className={`w-20 text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                                                            Endpoint
                                                        </label>
                                                        <input
                                                            id={`${providerId}-rest-endpoint`}
                                                            type="text"
                                                            placeholder={
                                                                providerConfig.type === ProviderTypeEnum.Ollama
                                                                    ? '/api/chat'
                                                            ...
                                                            value={providerConfig.restEndpoint || ''}
                                                            onChange={e => handleRestEndpointChange(providerId, e.target.value)}
                                                            className={`flex-1 rounded-md border text-sm ${isDarkMode ? 'border-slate-600 bg-slate-700 text-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-800' : 'border-gray-300 bg-white text-gray-700 focus:border-blue-400 focus:ring-2 focus:ring-blue-200'} p-2 outline-none`}
                                                        />
                                                    </div>
                                                    <div className="ml-20">
                                                        <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                                            REST API endpoint path (e.g., /chat/completions, /v1/chat/completions)
                                                        </p>
                                                    </div>
                                                </div>
                                            )}
                                        ...
                                        {(providerConfig.type === ProviderTypeEnum.CustomOpenAI ||
                                            ...
                                            providerConfig.type === ProviderTypeEnum.Llama) && (
                                                <div className="flex items-center">
                                                    <div className="w-20"></div> {/* Spacer to align with other fields */}
                                                    <button
                                                        type="button"
                                                        onClick={() => handleTestConnection(providerId)}
                                                        disabled={testingConnection[providerId] || !providerConfig.baseUrl?.trim()}
                                                        className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${testingConnection[providerId] || !providerConfig.baseUrl?.trim()
                                                                ? isDarkMode
                                                        ...
                                                        {testingConnection[providerId] ? 'Testing...' : 'Test Connection'}
                                                    </button>
                                                    {connectionResults[providerId] && (
                                                        <div className={`ml-3 text-sm ${connectionResults[providerId].success
                                                                ? 'text-green-500'
                                                                : 'text-red-500'
                                                            }`}>
                                                            {connectionResults[providerId].success
                                                                ? '✓ Connection successful'
                                                                : `✗ ${connectionResults[providerId].error}`}
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                                            ...
                                                            if (value) {
                                                                try {
                                                                    ...
                                                                    if (typeof headers === 'object' && headers !== null && !Array.isArray(headers)) {
                                                                        // Valid JSON - update the stored headers and reformat the text
                                                                        handleCustomHeadersChange(providerId, headers);
                                                                        const formattedValue = JSON.stringify(headers, null, 2);
                                                                        setHeaderTextValues(prev => ({
                                                                            ...prev,
                                                                            [providerId]: formattedValue,
                                                                        }));
                                                                    } else {
                                                                        // Invalid JSON object - reset to last valid value
                                                                        const validValue = providerConfig.customHeaders
                                                                            ? JSON.stringify(providerConfig.customHeaders, null, 2)
                                                                            : '';
                                                                        setHeaderTextValues(prev => ({
                                                                            ...prev,
                                                                            [providerId]: validValue,
                                                                        }));
                                                                    }
                                                                } catch (error) {
                                                                    // Invalid JSON syntax - reset to last valid value
                                                                    const validValue = providerConfig.customHeaders
                                                                        ? JSON.stringify(providerConfig.customHeaders, null, 2)
                                                                        : '';
                                                                    setHeaderTextValues(prev => ({
                                                                ...
                                                                // Empty value - clear headers
                                                                handleCustomHeadersChange(providerId, {});
                                                            }
                                                        }}
                                                        rows={3}
                                                        className={`w-full rounded-md border text-sm ${isDarkMode ? 'border-slate-600 bg-slate-700 text-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-800' : 'border-gray-300 bg-white text-gray-700 focus:border-blue-400 focus:ring-2 focus:ring-blue-200'} p-2 outline-none font-mono`}
                                                    />
                                                </div>
                                            </div>
                                            <div className="ml-20">
                                        ...
                                        <div className="flex flex-col space-y-2">
                                                            ...
                                                            if (value) {
                                                                try {
                                                                    ...
                                                                    if (typeof query === 'object' && query !== null && !Array.isArray(query)) {
                                                                        // Valid JSON - update the stored query and reformat the text
                                                                        handleCustomQueryChange(providerId, query);
                                                                        const formattedValue = JSON.stringify(query, null, 2);
                                                                        setQueryTextValues(prev => ({
                                                                            ...prev,
                                                                            [providerId]: formattedValue,
                                                                        }));
                                                                        ...
                                                                        // Invalid JSON object - reset to last valid value
                                                                        const validValue = providerConfig.customQuery
                                                                            ? JSON.stringify(providerConfig.customQuery, null, 2)
                                                                            : '';
                                                                        setQueryTextValues(prev => ({
                                                                            ...prev,
                                                                            [providerId]: validValue,
                                            ...
                                            <div className="ml-20">
                                                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                                    Add custom URL query parameters for API requests (optional)
                                                </p>
                                            </div>
                                        </div>

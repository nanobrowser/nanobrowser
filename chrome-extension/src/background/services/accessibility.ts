...
export class AccessibilityService {
  ...
  private async runEnhancedPipeline(
    ...
    try {
      ...
      const topImages = scorer.selectTopImages(contextualizedImages, 8);

      // Calculate metrics
      const processingTime = Date.now() - startTime;
      // const visionEnabledCount = contextualizedImages.filter(img => img.visionContext).length;
      ...
      const metrics: AnalysisQualityMetrics = {
        ...
        qualityLevel: this.determineQualityLevel(0, topImages.length),
      };
